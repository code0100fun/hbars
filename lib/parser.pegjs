...
attributeHelper
  = '{' _ name:ident _ m:mustacheContent? _ '}'
  {
...
quoted
  = chars:(("'" singleQuoteContent "'") / ('"' doubleQuoteContent '"') ) { return chars.join('') }
...
interpolation
  = '#{' t:mustacheContent? '}'
  { return { type: 'expression', content: t }; }
